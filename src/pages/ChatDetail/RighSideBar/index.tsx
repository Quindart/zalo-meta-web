...
const RightSideBar = () => {
  ...
  const { me } = userStore;
  const { channel, leaveRoom, dissolveGroup, deleteAllMessages } =
    useChatContext();
  ...
  useEffect(() => {
    if (channel && channel.members) {
      ...
      setIsGroupChannel(channel.members.length > 2);
    }
  }, [channel]);
  useEffect(() => {
  ...
  const AvatarChannel = () => {
    if (channel) {
      if (channel.type === "group") {
        ...
        return <Avatar src={channel.avatar} sx={{ width: 56, height: 56 }} />;
      }
    }
  };
  ...
  return showMembersPage && isGroupChannel ? (
    ...
    <Drawer variant="permanent" anchor="right" sx={{ flexShrink: 0 }}>
            ...
            <>
              ...
              <List sx={{ bgcolor: "white", borderRadius: 1 }}>
                ...
                >
                  <ListItemText
                    sx={{ ml: 2 }}
                    primary={"Xóa lịch sử trò chuyện"}
                  />
                </ListItemButton>
                {channel &&
                  channel.type === "group" &&
      ...
      <Dialog
        ...
        PaperProps={{
          sx: { borderRadius: 2, width: { xs: "90%", sm: 400 }, maxWidth: 400 },
        }}
      >
        <DialogTitle
        ...
        <DialogContent sx={{ px: 3 }}>
          <DialogContentText
            sx={{ textAlign: "center", fontSize: 15, color: "#343a40" }}
          >
      ...
      >
        ...
        <DialogContent sx={{ px: 3 }}>
          <DialogContentText
            sx={{ textAlign: "center", fontSize: 15, color: "#343a40" }}
          >
            Bạn có chắc chắn muốn xóa toàn bộ lịch sử trò chuyện? Hành động này
            không thể hoàn tác.
          </DialogContentText>
        </DialogContent>
        <DialogActions sx={{ justifyContent: "center", pb: 3, px: 2, gap: 1 }}>
